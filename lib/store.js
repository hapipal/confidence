...
internals.Store.prototype.write = function (key, value, next) {
    ...
    internals.parseKey(key, /\/((?:\$default)|(?:\w+))/g, function (err, path) {
        ...
        if (!node) {
            return next(new Error('Key path does not exist'));
        }

        // TODO: Check if change is going to create an invalid node

        node[path[il - 1]] = value;
        return next();
    });
};
...
internals.parseKey = function (key, exp, next) {
    ...
    if (key !== '/') {
        ...
        if (invalid) {
            return next(new Error('Bad key segment: ' + invalid));
        }
    }
