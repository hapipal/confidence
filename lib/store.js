...
exports = module.exports = internals.Store = function (document, options) {
...
internals.Store.prototype.get = function (key, criteria, applied) {
    ...
    return internals.walk(node, criteria, applied, this._options);
};
...
internals.walk = function (node, criteria, applied, options) {

    if (!node ||
        typeof node !== 'object') {

        return options.replaceValue ? options.replaceValue(node, criteria) : node;
    }

    if (node.hasOwnProperty('$value')) {
        return internals.walk(node.$value, criteria, applied, options);
    }
    ...
    for (let i = 0; i < keys.length; ++i) {
        ...
        const value = internals.walk(child, criteria, applied, options);
        if (value !== undefined) {
            parent[key] = value;
        }
    }
